...
Improved online knowledge distillation with JSD (Jensen-Shannon Divergence)
instead of forward KL. This addresses the distribution mismatch problem when
distilling from larger teachers (e.g., 4B) to smaller students (e.g., 0.8B).

Key changes from train_online_distill.py:
1. Uses JSD instead of forward KL (more stable for large teacher-student gaps)
2. Higher default temperature (2.0 vs 1.0) to smooth teacher distribution
3. Higher default alpha (0.7 vs 0.3) to weight KD loss more heavily

Usage:
    # First, SFT-train a LoRA on the teacher (e.g. 4B model):
    python train_sft.py --dataset tooluse --model_path Qwen/Qwen3.5-4B \
        --output_dir ./qwen-4b-expert --epochs 3 --batch_size 1 --grad_acc 16

    # Then distill to 0.8B using the trained teacher:
    python distill_improved.py --dataset tooluse \
        --teacher_model Qwen/Qwen3.5-4B \
        --teacher_lora_path ./qwen-4b-expert \
        --dataset_path data/tooluse_data/train_data_fixed \
        --output_dir ./qwen-distilled-improved
"""
...
# ── Improved distillation trainer with JSD ────────────────────────────────────

class ImprovedDistillationTrainer(SFTTrainer):
    def __init__(self, teacher_model, alpha=0.7, temp=2.0, jsd_alpha=0.5, *args, **kwargs):
        """
        Args:
            teacher_model: Teacher model for distillation.
            alpha: Weight for KD loss (0 = CE only, 1 = KD only).
            temp: Temperature for logit scaling. Higher = smoother distribution.
            jsd_alpha: JSD mixture coefficient (0 = forward KL, 1 = reverse KL, 0.5 = JSD).
        """
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if response_positions[0].numel() > 0:
            ...
            # jsd_alpha=0: Forward KL (standard)
            # jsd_alpha=1: Reverse KL
            # jsd_alpha=0.5: Standard JSD
            jsd_alpha = torch.tensor(self.jsd_alpha, dtype=t_log_probs.dtype)

            # Compute mixture distribution: log(a * p + (1-a) * q)
            mixture_log_probs = torch.logsumexp(
                torch.stack([
                    s_log_probs + torch.log(1 - jsd_alpha),
                    t_log_probs + torch.log(jsd_alpha)
                ]),
                dim=0,
            )

            # KL(p_mixture || p_teacher) and KL(p_mixture || p_student)
            kl_teacher = F.kl_div(mixture_log_probs, t_log_probs, reduction="batchmean", log_target=True)
            kl_student = F.kl_div(mixture_log_probs, s_log_probs, reduction="batchmean", log_target=True)

            # JSD = alpha * KL_teacher + (1 - alpha) * KL_student
            loss_kd = (jsd_alpha * kl_teacher + (1 - jsd_alpha) * kl_student) * (self.temp ** 2)
...
def main():
    ...
    parser.add_argument("--teacher_lora_path",  type=str, default=None,
                        help="Path to fine-tuned teacher LoRA (from train_sft.py output). "
                             "If not set, uses the base teacher model without LoRA.")
    parser.add_argument("--student_model",      type=str, default="Qwen/Qwen3.5-0.8B")
    parser.add_argument("--output_dir",         type=str, default="./qwen-distilled-improved")
    ...
    parser.add_argument("--alpha",              type=float, default=0.7,
                        help="Weight for KD loss (0 = CE only, 1 = KD only). Default 0.7.")
    parser.add_argument("--temp",               type=float, default=2.0,
                        help="Temperature for logit scaling. Higher = smoother. Default 2.0.")
    parser.add_argument("--jsd_alpha",          type=float, default=0.5,
                        help="JSD mixture coefficient (0 = forward KL, 1 = reverse KL, 0.5 = JSD). Default 0.5.")
    ...
    print(f"Starting improved distillation training (alpha={args.alpha}, temp={args.temp}, jsd_alpha={args.jsd_alpha}) ...")
